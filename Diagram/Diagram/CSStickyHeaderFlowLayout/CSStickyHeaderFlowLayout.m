...
-(id)init
{
    self = [super init];
    if (self) {
        self.itemSize = CGSizeMake(DataCenterCellWidth+25,(MainScreenHeight-301)/2);
        self.scrollDirection = UICollectionViewScrollDirectionHorizontal;
        self.sectionInset = UIEdgeInsetsMake(0,-23, 0, 0);
        self.minimumInteritemSpacing = 1;
        self.minimumLineSpacing = 1;
        self.headerReferenceSize = CGSizeMake(1, MainScreenWidth);
    }
    return self;
}
...
- (void)updateHeaderAttributes:(UICollectionViewLayoutAttributes *)attributes lastCellAttributes:(UICollectionViewLayoutAttributes *)lastCellAttributes
{
    ...
    CGFloat sectionMaxY = CGRectGetMaxX(lastCellAttributes.frame) - attributes.frame.size.width;
    CGFloat y = CGRectGetMaxX(currentBounds) - currentBounds.size.width + self.collectionView.contentInset.right;
    
    if (self.parallaxHeaderAlwaysOnTop) {
        y += self.parallaxHeaderMinimumReferenceSize.width;
    }
    
    CGFloat maxY = MIN(MAX(y, attributes.frame.origin.x), sectionMaxY);
    origin.x = maxY;
